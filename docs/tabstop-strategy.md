...
# TabStop 跳转策略模式重构评估与设计

## 当前实现分析

### 现有跳转机制

- **位置**: `src/snippetManager.ts` 中的 `selectNextTabStop()` 和 `selectPrevTabStop()`
- **逻辑**: 基于 `session.currentIndex` 的简单数值递增/递减查找
- **限制**: 
